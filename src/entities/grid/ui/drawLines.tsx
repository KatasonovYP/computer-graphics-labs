...
function drawLines(stage: IStage) {
	...
	for (let stepY = getNear(-y, gridSize); stepY < -y + height; stepY += gridSize) {
		gridLines.push(<Line points={[-x, stepY, -x + width, stepY]} stroke="#aaa" strokeWidth={1} key={key++} />);
	}

	for (let stepX = getNear(-x, gridSize); stepX < -x + width; stepX += gridSize) {
		gridLines.push(<Line points={[stepX, -y, stepX, -y + height]} stroke="#aaa" strokeWidth={1} key={key++} />);
	}
